...
class RegionDataSource(var retrofitClient:RetrofitClient) : PageKeyedDataSource<Int,Results>() {
    ...
    override fun loadInitial(
        ...
        CoroutineScope(Dispatchers.IO).launch {
            retrofitClient.getAPI(ApiEndPoints::class.java).getNowPlayingRegion(FIRST_PAGE, region!!)
    ...
    override fun loadBefore(params: LoadParams<Int>, callback: LoadCallback<Int, Results>) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            retrofitClient.getAPI(ApiEndPoints::class.java).getNowPlayingRegion(params.key,region!!)
                    .enqueue(object : Callback<Movies> {
                        override fun onResponse(call: Call<Movies>, response: Response<Movies>) {
                            if (response.isSuccessful){
                                val apiResponse = response.body()!!
