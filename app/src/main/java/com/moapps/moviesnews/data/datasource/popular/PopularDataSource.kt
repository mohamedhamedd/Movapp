...
class PopularDataSource(var retrofitClient: RetrofitClient) : PageKeyedDataSource<Int, Results>() {
        ...
        GlobalScope.launch(Dispatchers.IO) {
            retrofitClient.getAPI(ApiEndPoints::class.java).getMovies(TOPIC, params.key)
                    .enqueue(object : Callback<Movies> {
                        override fun onResponse(call: Call<Movies>, response: Response<Movies>) {
                            if (response.isSuccessful) {
                                val responseItems = response.body()!!.results
    ...
    override fun loadAfter(params: LoadParams<Int>, callback: LoadCallback<Int, Results>) {
        ...
        GlobalScope.launch(Dispatchers.IO) {
            val response = retrofitClient.getAPI(ApiEndPoints::class.java).getMovies(TOPIC, params.key)
                    .enqueue(object : Callback<Movies>{
                        override fun onResponse(call: Call<Movies>, response: Response<Movies>) {
                            if (response.isSuccessful) {
                                val apiResponse = response.body()!!
